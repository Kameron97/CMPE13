...
int main(void)
{
    ...
    while(1) {
        if (adcResult.event == 1){
            double percentage = ((double)adcResult.value / 1023.0) * 100.0;
            sprintf(outcome, "Potentiometer Value:\n %4d\n %3.0f%%", adcResult.value, percentage);
            OledDrawString(outcome);
            OledUpdate();
            adcResult.event = 0;
        }
    }
...
void __ISR(_ADC_VECTOR, IPL2AUTO) AdcHandler(void)
{
    ...
    if(avg != adcResult.value){
        adcResult.event = 1;
        adcResult.value = (uint16_t) avg;
